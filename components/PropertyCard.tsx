...
const PropertyCard = ({ twoColumns }: twoColType) => {
    return (
        ...
        <div className={`max-w-sm xl:max-w-md bg-white border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700  `}>
            ...
            <div className={`p-5 flex flex-col gap-${twoColumns ? '0' : 3}`}>
                <h1 className={`mb-2 ${twoColumns ? 'text-[13px] font-semibold uppercase' : null} text-xl md:text-2xl font-bold tracking-tight text-gray-900 dark:text-white`}>Three Bedroom houses near Oloibon</h1>
                <div className='flex gap-2 items-center'>
                    <Image src={location} alt='pin-location' className='w-8' />
                    <h2 className='text-gray-300 uppercase'>Rongai, Tuskys</h2>
                </div>
                <p className="mb-3 font-semibold text-white dark:text-gray-300 text-lg">Ksh. 125000 per month</p>
